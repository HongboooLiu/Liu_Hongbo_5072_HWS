...
def recomm_news():
  ...
  recommendation_news = requests.get("https://v3api.dmzj.com/article/recommend/header.json").json()
  
  return recommendation_news

def get_comic_recomm(): 
...
def search(big_cat_id,keywords,page):
  ...
  search = requests.get(search).json()
  return search
...
def get_description(cat_type,id):
  ...
  description = "https://v3api.dmzj.com/{0}/{0}.json".format(cat_type,id)
  description = requests.get(description).json()
  return description